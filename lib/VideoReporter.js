...
VideoReporter.prototype._singleVideoPath = function(result) {
  ...
  if (self.options.singleVideoPath === 'uuid') {
    ...
    return SanitizeFilename(result.fullName + process.env.E2E_SUITE + '.mp4');
  } else {
...
VideoReporter.prototype.specDone = function(result) {
  ...
  if (!self.options.singleVideo) {
    ...
    var text = '<font size="8">' + date.toLocaleString() + '</font> ';
...
VideoReporter.prototype.jasmineStarted = function() {
  var self = this;
  if (self.options.singleVideo) {
    var videoPath = Path.join(self.options.baseDirectory, 'protractor-specs-' + process.env.E2E_SUITE + '.mp4');
...
VideoReporter.prototype.jasmineDone = function() {
  ...
  if (self.options.singleVideo) {
    ...
    if (self.options.createSubtitles) {
      Fs.writeFileSync(
          Path.join(self.options.baseDirectory, 'protractor-specs-' + process.env.E2E_SUITE + '.srt'),

...
VideoReporter.prototype._singleVideoPath = function(result) {
  ...
  if (self.options.singleVideoPath === 'uuid') {
    ...
    return SanitizeFilename(result.fullName + process.env.ORG_NUM + '.mp4');
  } else {
...
VideoReporter.prototype.specDone = function(result) {
  ...
  if (!self.options.singleVideo) {
    ...
    var indy = new Date(utc + (3600000 * indyOffset));

    var text = '<font size="8">' + indy.toLocaleString() + '</font> ';
    text += ' <font size="8">' + result.description + '</font>';
    switch (result.status) {
      case 'passed':
        text += ' <font size="8" color="#31D57C">SUCCESS</font>';
        break;
      case 'failed':
        text += ' <font size="8" color="red">FAILED</font>';
        break;
      case 'pending':
        text += ' <font size="8" color="yellow">PENDING</font>';
        break;
    }
...
VideoReporter.prototype.jasmineStarted = function() {
  var self = this;
  if (self.options.singleVideo) {
    var videoPath = Path.join(self.options.baseDirectory, 'protractor-specs' + process.env.ORG_NUM + '.mp4');
    self._startScreencast(videoPath);
...
VideoReporter.prototype.jasmineDone = function() {
  ...
  if (self.options.singleVideo) {
    ...
    if (self.options.createSubtitles) {
      Fs.writeFileSync(
          Path.join(self.options.baseDirectory, 'protractor-specs' + process.env.ORG_NUM + '.srt'),
